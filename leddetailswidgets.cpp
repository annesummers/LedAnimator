...
LedDetailsWidget::LedDetailsWidget(QWidget *parent, Animation &animation, Led& led, LedContainerWidget& ledGroup) :
    QWidget(parent),
    iPositionLabel(NULL),
    iCurrentFrameWidget(NULL),
    iFramesListWidget(NULL) {

    resize(parentWidget()->width(), 30);
...
LedDetailsListWidget::LedDetailsListWidget(QWidget *parent, Animation &animation) :
...
void LedDetailsListWidget::addLed(Led &led) {
    if(!iShownLeds.contains(&led)) {
        ...
        qDebug("add new led, %d, %d", led.row(), led.column());
    }
}
